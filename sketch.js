...
function setup() {
  ...
  tierra=createSprite(width-100,50,10,10);
  ...
  //rex.setCollider("rectangle",0,0,500,rex.height)
  gameover=createSprite(width/2,height/2,30,30);
  gameover.addImage("fin",over);
  reiniciar=createSprite(width/2,height-250,30,30);
...
function draw() {
  ...
  if(touches.length>0||keyDown("space")&&rex.y>=150){
  rex.velocityY=-15;
  jump.play();
  touches=[];
  }
